...
usage: titan-gc-prepare [-h] [-f STR] [--fastq_separator STR] [--fastq_pattern STR] [--pe1_pattern STR] [--pe2_pattern STR]
                        [-a STR] [--assembly_pattern STR] [-r] [--prefix STR] [--tsv] [--pangolin_docker STR] [--params STR]
...
Titan-GC Prepare Parameters:
  ...
  WORKFLOW              The Titan-GC workflow to use for analysis. Options: clearlabs, fasta, illumina_pe, illumina_se, ont
  ...
  -a STR, --assembly_ext STR
                        Extension of the FASTA assemblies. Default: .fasta
  --assembly_pattern STR
                        Glob pattern to match assembly FASTAs. Default: *.fasta
...
Heavily based off bactopia prepare (https://github.com/bactopia/bactopia/blob/master/bin/bactopia/bactopia-prepare.py)
"""
...
if __name__ == '__main__':
    ...
    group1.add_argument(
        'workflow', metavar='WORKFLOW', type=str, choices=['clearlabs', 'fasta', 'illumina_pe', 'illumina_se', 'ont'],
    ...
    group1.add_argument(
        '-a', '--assembly_ext', metavar='STR', type=str, default=".fasta",
        help='Extension of the FASTA assemblies. Default: .fasta'
    )
    group1.add_argument(
        '--assembly_pattern', metavar='STR', type=str, default="*.fasta",
        help='Glob pattern to match assembly FASTAs. Default: *.fasta'
    )
    group1.add_argument(
    ...
    if total_samples:
        ...
        print(f"Unable to find any FASTQs or FASTAs in {args.path}. Please try adjusting the parameters to fit your needs.", file=sys.stderr)
        ...
        print(f"    --assembly_pattern => {args.assembly_pattern}", file=sys.stderr)
