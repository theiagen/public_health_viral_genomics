...
Mercury_PE_Prep
=================
The Mercury_PE_Prep workflow was written to process paired-end read data, assembly files, and contextual metadata to prepare submission for samples individually. More information on `required user inputs <tables/mercury_workflows/mercury_pe_prep_required_inputs.csv>`_, `optional user inputs <tables/mercury_workflows/mercury_pe_prep_optional_inputs.csv>`_, and the `outputs <tables/mercury_workflows/mercury_pe_prep_outputs.csv>`_ generated by Mercury_PE_Prep are outlinedin the links.
...
Mercury_SE_Prep
================
The Mercury_SE_Prep workflow was written to process single-end read data, assembly files, and contextual metadata to prepare submission for samples individually. More information on `required user inputs <tables/mercury_workflows/mercury__se_prep_required_inputs.csv>`_, `optional user inputs <tables/mercury_workflows/mercury_se_prep_optional_inputs.csv>`_, and the `outputs <tables/mercury_workflows/mercury_se_prep_outputs.csv>`_ generated by Mercury_PE_Prep are outlinedin the links.
