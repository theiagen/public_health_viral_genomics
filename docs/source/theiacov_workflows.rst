...
--------------------------------------------------
It is an increasingly critical function for public health laboratories around the world to undertake genomic characterization of pathogens. The TheiaCoV workflows for genomic characterisation make this easy by providing workflows that take raw next-generation sequencing (NGS) data and characterise SARS-CoV-2 to the most up-to-date international standards. These workflows will
...
* Illumina paired-end (TheiaCoV_Illumina_PE_ workflow_)
* Illumina single-end (TheiaCoV_Illumina_SE_ workflow_)
* Clear Labs (TheiaCoV_ClearLabs_ workflow_)
* Oxford Nanopore Technology (TheiaCoV_ONT_ workflow_)

The major differences between these four workflows are in how the read mapping, primer trimming, and consensus genome calling are performed due to differences in input file types. The fifth TheiaCoV Genomic Characterization WDL workflow, TheiaCoV_FASTA_, takes assembled SARS-CoV-2 genomes as its input then produces assembly QC metrics, Pangolin lineage and clade assignments only. Methodological details of these workflow and information on how to use and apply these workflows for public health investigations are available in the linked sections below.
